                        ...
                        <template v-else>
                            <tr v-for="country in paginatedCountries" :key="country.name">
                                <td>
                                    <div class="d-flex px-2 py-1">
                                        <div>
                                            <img :src="country.flag" class="avatar avatar-sm me-3"
                                                :alt="country.name" />
                                        </div>
                                        <div class="d-flex flex-column justify-content-center">
                                            <router-link :to="`/leagues/${country.id}`"
                                ...
                                <td class="align-middle text-center text-sm">
                                    <span
                                        :class="['badge badge-sm', country.isactive === true ? 'bg-gradient-success' : 'bg-gradient-secondary']">
                                        {{ country.isactive }}
                                    </span>
                                </td>
                                <td class="align-middle text-center">
                                    <span class="text-secondary text-xs font-weight-bold">{{ country.total_leagues
                                        }}</span>
                                </td>
                            </tr>
...
export default {
    data() {
        return {
            countries: [], // Holds data from the API
    ...
    methods: {
        async fetchCountries() {
            try {
                const response = await axios.get("http://localhost:5001/api/countries"); // Update URL as needed
